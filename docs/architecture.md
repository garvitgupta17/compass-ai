...
- **Roadmap Engine (`core/roadmap.py`)**: Schedules skills into weekly blocks respecting available study hours (`hours_per_week`) and overall target deadline (`deadline_weeks`). Note: The current roadmap prioritizes prerequisite ordering and sequential skill progression rather than solving a full bin-packing optimization problem for weekly capacity.
...
- **Filtering & Ranking Engine (`rag/ranking.py`)**: Enforces hard constraint filters (budget: free/paid, language: English/Hindi) and applies multi-factor hybrid scoring:
  $$\text{Final Score} = 0.30 \cdot \text{Vector Sim} + 0.25 \cdot \text{Skill Match} + 0.15 \cdot \text{Budget Fit} + 0.15 \cdot \text{Language Fit} + 0.10 \cdot \text{Format Fit} + 0.05 \cdot \text{Exposure Bonus}$$
  *Skill Match distinction*: Explicitly distinguishes between Exact Skill Matches (score 1.0) and Related/Substring Skill Matches (score 0.7) in preference scoring.

---

## 4. Key Technical Decisions
1. **Google Gemini API (`gemini-3.6-flash`) Integration & Embeddings**: Generates grounded explanations and conversational responses via `google-genai` SDK. When API access is unavailable, a deterministic hash-based vector representation is provided as an offline/testing fallback so that core workflows remain testable without external API access.
...
5. **Grounded Resource Recommendations**: Resource titles, providers, and URLs are selected exclusively from the curated resource dataset (`data/resources.csv`), while Gemini is restricted to generating explanations from deterministic context.
